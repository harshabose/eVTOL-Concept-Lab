 ...
 *
 * The opt namespace encapsulates utilities and structures related to gradient descent optimisation.
...
namespace opt {
    ...
    struct function_wrapper {
        ...
        requires (meta_checks::check_func_v<returnType, funcType, argType...>)
        ...
        requires (std::is_same_v<meta_checks::remove_all_qual<tupleType>, std::tuple<argType...>>)
        [[nodiscard]] returnType eval_func_at (tupleType &&IN_ARGS) const {
        ...
        [[nodiscard]] returnType eval_func_at (argTypes_&&... IN_ARGS) const {
    ...
    public:
        ...
        requires(meta_checks::are_same<argType_..., argType...>::value && meta_checks::check_func_v<returnType, funcType_, argType_...>)
        explicit gradient_decent (funcType_ &&IN_FUNC, argType_ &&... IN_GUESS) {
            this->function = std::make_unique<opt::function_wrapper<returnType, argType...>>(
         ...
         */
        template<class tupleType>
        requires(meta_checks::are_tuples_same_v<tupleType, std::tuple<argType...>>)
        ...
        requires(meta_checks::are_same<argType_..., argType...>::value)
 ...
 *
         ...
         * @note This method is marked as noexcept
 */
        template<class type>
        requires (std::is_same_v<meta_checks::remove_all_qual<type>, returnType>)
        ...
        template<class type>
        requires (std::is_same_v<meta_checks::remove_all_qual<type>, returnType>)
        void set_initial_finite_difference (type &&IN_DIFF) {
            this->finite_difference_step = std::forward<type>(IN_DIFF);
        }
         ...
         * @note This method is marked as noexcept
         */
        void toggle_classic_gradient_algo () noexcept {
         ...
         * @note This method is marked as noexcept
         */
        void toggle_derivative_scaling () noexcept {
        ...
        std::pair<returnType, std::tuple<argType...>> perform_gradient_decent () {
            ...
            try {
                ...
                std::cerr << "GD Exception Encountered..." << std::endl;
                std::cerr << "Re-declaring Optimal Value to MAX... EXITING..." << std::endl;
                this->optimal_val = std::numeric_limits<returnType>::max();
                std::cerr << e.what() << std::endl;
            }

            if (eval >= this->max_eval && this->current_tolerance > this->tolerance) {
                std::cerr << "Unable to converge GD with given settings..." << std::endl;
    ...
    protected:
         ...
         */
        std::unique_ptr<opt::function_wrapper<returnType, argType...>> function;
        /**
         ...
         */
        template<class tupleType>
        returnType eval_func_at (tupleType&& IN_ARGS) {
         ...
         */
        void step_forward_with_secant_method (std::tuple<argType...> IN_POINT) {
        ...
        void set_high_derivatives_helper () noexcept {
         ...
         */
        template <std::size_t... i>
        void set_high_derivatives (std::index_sequence<i...>) noexcept {
        ...
        auto calculate_derivatives_at_helper (tupleType &&IN_TUPLE, std::index_sequence<i...>) {
            auto find_derivative_at = [this] <std::size_t i_, std::size_t... index> (tupleType &&IN_TUPLE_, std::index_sequence<index...>) -> meta_checks::tuple_args_type_at<i_, tupleType> {
                meta_checks::tuple_args_type_at<i_, tupleType> result{};
                try {
         ...
         */
        template <std::size_t... i>
        bool check_point_bounds (std::index_sequence<i...>) noexcept {
         ...
         */
        template<class tupleType, std::size_t... i>
        void verbose_print_tuple (tupleType&& IN_TUPLE, std::index_sequence<i...>) noexcept {
#if VERBOSITY
         ...
         */
        template <std::size_t... i>
        auto get_distance_tuple (std::tuple<argType...> &IN_FIRST_TUPLE, std::tuple<argType...> &IN_SEC_TUPLE, std::index_sequence<i...>) noexcept {
         ...
         * @note This method is marked as noexcept
         */
        returnType get_tolerance () noexcept {
         ...
         * @note This method is marked as noexcept
         */
        returnType scale_function (const returnType IN_X, const returnType IN_FACTOR) noexcept {
         ...
         */
        template <std::size_t... i>
        void scale (returnType* IN_TO_SCALE, std::index_sequence<i...>) noexcept {
         ...
         * </ul>
         */
        returnType secant_learning_rate_scaling (returnType IN_CURRENT_VAL, const returnType IN_REQUIRED_VAL) {
