...
struct create_constraint {
    ...
    requires (meta_checks::check_func_v<value_type_, constraint_func_types_, args_type...> && meta_checks::is_string_v<operator_type_> && std::is_floating_point_v<tolerance_type_>)
...
public:
    ...
    requires (meta_checks::check_func_v<return_type, func_type_, args_type...>)
    ...
    requires (meta_checks::are_same<std::tuple<args_type_...>, std::tuple<args_type...>>::value)
...
protected:
    ...
    struct constraint_manager final : constraint_manager_base {
        ...
        explicit constraint_manager (constraint_func_types... IN_FUNCS, value_types&&... IN_VALUES) {
            ...
            auto create = [this, &IN_FUNCS...] <std::size_t... i> (std::index_sequence<i...>) {
                auto create_at = [this] <std::size_t i_, class constraint_func_type> (constraint_func_type &IN_FUNC) -> void {
                    using return_at = return_type_t<constraint_func_type>;
                    auto RETURN_POINTER = std::make_shared<return_at>();
                    std::get<i_>(this->return_tuple) = RETURN_POINTER;

                    auto constraint_at = [FUNC = std::forward<constraint_func_type>(IN_FUNC), RETURN_POINTER = RETURN_POINTER] (args_type... args) -> void {
            ...
            create(std::index_sequence_for<constraint_func_types...>{});
        ...
        void add_constraint_values (value_types&&... IN_VALUES) {
            ...
            auto add = [this, &tuple] <std::size_t...i_> (std::index_sequence<i_...>) {
                auto add_at = [this, &tuple] <std::size_t i> () {
                    try {
                        if(std::is_same_v<meta_checks::tuple_args_type_at<i, decltype(this->constraint_values)>, meta_checks::tuple_args_type_at<i, std::tuple<value_types...>>>) {
    ...
    static void check_bounds (tuple_type &&IN_LOWER_BOUNDS, tuple_type &&IN_UPPER_BOUNDS) {
        ...
        if (meta_checks::is_tuple_default_initialized(IN_UPPER_BOUNDS) || meta_checks::is_tuple_default_initialized(IN_LOWER_BOUNDS)) {
...
//                std::cout << "fitness: " << this->fitness_table.at(i - 1) << std::endl;
    ...
    bool survivor_selector (const bool IN_STANDARD_DEVIATION) {
        ...
        if (crossover_mutation_ == true) { //crossover
            ...
            VERBOSE_PRINT("LEAST FIT CHROMOSOMES DELETED with fitness: " << this->fitness_table[first_child_index] << " and " << this->fitness_table[second_child_index]);
